...
import { PageTwo } from '@/components/report/page-two'
import { AssistantPanel } from '@/components/ai/assistant-panel'

export default async function ReportPageTwo({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <LangProvider><AppShell user={{ name: session.user.name, email: session.user.email }} currentReportId={id}><ReportProvider id={id} initialData={report.data} initialStatus={report.status}><div className="report-workspace"><div className="report-main"><ReportToolbar /><PageTwo /></div><AssistantPanel /></div></ReportProvider></AppShell></LangProvider>
}
