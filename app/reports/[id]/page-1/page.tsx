...
import { PageOne } from '@/components/report/page-one'
import { AssistantPanel } from '@/components/ai/assistant-panel'

export default async function ReportPageOne({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <LangProvider><AppShell user={{ name: session.user.name, email: session.user.email }} currentReportId={id}><ReportProvider id={id} initialData={report.data} initialStatus={report.status}><div className="report-workspace"><div className="report-main"><ReportToolbar /><PageOne /></div><AssistantPanel /></div></ReportProvider></AppShell></LangProvider>
}
