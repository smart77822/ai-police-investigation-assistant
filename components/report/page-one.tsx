...
export function PageOne({ editable = true }: { editable?: boolean } = {}) {
  ...
  return <ReportZimniBairuni data={form} readOnly={!editable} onChange={(next) => setMany([
    ...
    { path: 'page1.occurrencePlace', value: next.occurrenceDateTimePlace },
    { path: 'page1.offence', value: next.offenceSections },
    ...next.entries.map((entry, index) => ({ path: `page1.entries[${index}].actionDateTime`, value: entry.actionDateTime })),
    ...next.entries.map((entry, index) => ({ path: `page1.entries[${index}].serialNumber`, value: entry.reportNumber })),
    ...next.entries.map((entry, index) => ({ path: `page1.entries[${index}].investigationDetails`, value: entry.details })),
  ])} />
}
